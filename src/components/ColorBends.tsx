...
void main() {
  ...
  q += 0.18 * cos(t * 0.6) - 7.4;

  vec2 toward = (uPointer - rp);

  q += toward * uMouseInfluence * 0.16;
  ...
  if (uColorCount > 0) {
    ...
    for (int i = 0; i < MAX_COLORS; ++i) {
      ...
      float kBelow =
        clamp(uWarpStrength, 0.0, 1.0);

      float kMix = pow(kBelow, 0.35);
  ...
  if (uNoise > 0.0001) {
    float n =
      hash(gl_FragCoord.xy + vec2(uTime));
